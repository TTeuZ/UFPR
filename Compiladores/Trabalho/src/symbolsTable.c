...
void innitSymbolsTable() { symbolsTable.sp = 0; }

void cleanSymbolsTable() { removeSymbols(symbolsTable.sp); }
...
void removeSymbols(size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    symbol = symbolsTable.symbols[symbolsTable.sp - 1];
