...
/* Mesmo que ele não esteja no vetor!! */
/* para que ele se mantenha ordenado e retorna esta posição. */
int find_possible_pos (int x, conjunto_t *c) {
    int iterator = 0;
...
/* O maior conjunto é aquele que possui maior cardinalidade */
...
/* O menor conjunto é aquele que possui menor cardinalidade */
...
conjunto_t * cria_interseccao (conjunto_t *c1, conjunto_t *c2) {
    ...
    /* Verifica se a criação do conjunto para a diferença ocorreu corretamente */
    if ((conj = cria_conjunto (l_conj->max)) != NULL) {
        ...
        for (count = 0; count < l_conj->card; count++) {
            ...
            if (busca_binaria (*(l_conj->v+count), b_conj) == 1) 
                insere_conjunto (conj, *(l_conj->v+count));
            }
        return conj;
    } else return NULL;
}
...
conjunto_t * cria_subconjunto (conjunto_t *c, int n) {
    ...
    if ((conj = cria_conjunto (c->max)) != NULL) {
        ...
        while (count < n) {
            ...
            if (insere_conjunto (conj, *(c->v+(rand() % c->card)))) 
                count++;
        }
        return conj;
    } else return NULL;
    ...
    if ((conj = cria_conjunto (max)) != NULL) {
        ...
        for (count = 0; count < b_conj->card; count++) {
            ...
            if (count < b_conj->card)
                /* insere o elemento da vez no conjunto união ** regras do insere aplicadas ** */
                insere_conjunto (conj, *(b_conj->v+count));
        }
        return conj;
    } else return NULL;
}
