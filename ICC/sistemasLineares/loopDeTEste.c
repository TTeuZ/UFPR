  ...
  for (count = 0; count < 1; count++) {
    ...
    if (s_vet == NULL) {
      ...
      if (! (solVet = malloc (sizeof (real_t) * sizes[count]))) {
        ...
        if (eliminacaoGauss (s_vet, solVet, &time_vet_egp) == OPE_ERROR) 
          fprintf (stderr, "Erro de operaçôes - Infinito | NaN\n");
    ...
    if (s_pont == NULL) {
      ...
      if (! (solPont = malloc (sizeof (real_t) * sizes[count]))) {
        ...
        /* aplicação da eliminação de gauss */
        if (eliminacaoGauss (s_pont, solPont, &time_pont_egp) == OPE_ERROR) 
          fprintf (stderr, "Erro de operaçôes - Infinito | NaN\n");
  ...
  for (count = 0; count < 1; count++) {
    ...
    if (s_vet == NULL) {
      ...
      if (! (solVet = malloc (sizeof (real_t) * sizes[count]))) {
        ...
        /* aplicação da eliminação de gauss */
        if (eliminacaoGauss (s_vet, solVet, &time_vet_egp) == OPE_ERROR)
          fprintf (stderr, "Solução proporciou um NaN\n");
