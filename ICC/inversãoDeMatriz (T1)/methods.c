...
int lu_factorization (matrix_t *u_matrix, matrix_t *l_matrix, pivot_steps_t *pivot_steps, double *lu_time) {
    ...
    *lu_time = timestamp () - *lu_time;
    ...
    if (calc_determinant (u_matrix) == 0)
...
int matrix_refinement (matrix_t *inv_matrix, matrix_t *matrix, matrix_t *l_matrix, matrix_t *u_matrix, pivot_steps_t *steps, double *norma_vet, int iterations, double *iter_time, double *residue_time) {
    matrix_t *residue_matrix, *solution, *i_matrix;
    ...
    *residue_time = 0;
    ...
    if (! (solution = alloc_matrix (size)) || ! (residue_matrix = alloc_matrix (size)) || ! (i_matrix = alloc_matrix (size)))
        return ALLOC_ERROR;
    ...
    for (count = 0; count < iterations; count++) {
        ...
        apply_pivot_steps (count == 0  ? i_matrix : residue_matrix, steps);
        for (ls_count = 0; ls_count < size; ls_count++) {
            if (inv_retro_subs (l_matrix, count == 0  ? i_matrix : residue_matrix, temp_sol, ls_count) == NAN_INF_ERROR)
                return NAN_INF_ERROR;
        ...
        norma_vet[count] = normalization (residue_matrix, &act_residue_time);
