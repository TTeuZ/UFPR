...
graph_t *alloc_graph (int vertices_qtd) {
    ...
    if (! (graph->adj_matrix = malloc (sizeof (int) * vertices_qtd)))
        return NULL;
    if (! (graph->adj_matrix[0] = malloc (sizeof (int) * vertices_qtd * vertices_qtd)))
        return NULL;
    
    for (count = 0; count < vertices_qtd; count++)
        graph->adj_matrix[count] = graph->adj_matrix[count - 1] + vertices_qtd;
    
    // seta a matriz para 0
    memset (graph->adj_matrix, 0, (sizeof (int) * vertices_qtd * vertices_qtd));
    return graph;
}
