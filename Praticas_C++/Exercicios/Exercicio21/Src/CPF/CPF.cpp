...
bool CPF::operator==(const CPF& outro) const { return this->numero == outro.numero; }
...
unsigned short CPF::operator[](const int idx) const {
    if (idx < 0 || idx >= 11) throw std::out_of_range{"Posição indicada fora dos limites"};
...
uint64_t CPF::operator()(const int begin, const int qtd) const {
    if (begin < 0 || qtd < 0) throw std::invalid_argument{"Indices precisam ser positivos"};
    if ((qtd + begin) >= 11) throw std::out_of_range{"Posição indicada fora dos limites"};
...
CPF::operator unsigned long() const { return this->numero; }
