...
namespace ufpr {
// Constructors
SalaAula::SalaAula(const std::string& nome, const uint16_t capacidade) : nome{nome} { this->setCapacidade(capacidade); }
...
const std::list<Disciplina*>& SalaAula::getDisciplinas() const { return this->disciplinasMinistradas; }

// Setters
void SalaAula::setNome(const std::string& nome) { this->nome = nome; }

void SalaAula::setCapacidade(const uint16_t capacidade) {
    if (capacidade <= 0) throw exceptions::NaoPositivoException{"Capacidade nâo pode ser menor ou igual a zero"};
...
void SalaAula::adicionarDisciplina(Disciplina* disciplina) {
    if (disciplina == nullptr) throw exceptions::NullPointerException{"Disciplina não pode ser nula"};
